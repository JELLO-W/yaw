...
https://github.com/JELLO-W/yaw/blob/master/yaw_extensions/yaw_ros_interfaces
...
1. Copy the `yaw_ros_interfaces` folder to your ROS workspace (assumed to be `~/ros_ws`).
...
2. Compile the ROS interfaces:
    ...
    **Note**: If the wrong version of Python is used, the compilation will fail. Make sure that the correct version of cmake 
    is used by modifying the `cmake_minimum_required` version in the `~/ros_ws/src/yaw_ros_interfaces/CMakeLists.txt` file:
...
4. Verify that the ROS Audio message interface is compiled:
    ...
    ```bash
   [yaw_ros_interfaces/ROSAudioMessage]:
...
5. Verify that the ROS Audio service interface is compiled:
    ...
    ```bash
   [yaw_ros_interfaces/ROSAudioService]:
   string request
   ---
   yaw_ros_interfaces/ROSAudioMessage response
    ...
    ```
     ...
     Run your YAW enabled script from the same terminal. Now you can transmit ROS audio messages in PUB/SUB and REQ/REP.
