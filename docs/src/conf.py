...
# all_imported_modules = get_all_imports_in_package("yaw")
all_imported_modules = all_imported_modules_pre
mock_imports = [mod for mod in all_imported_modules if 'yaw.' not in mod]
...
\pretitle{%
  \begin{center}
  \vspace{\droptitle}
  \includegraphics[width=60mm]{../resources/yaw.png}\\[\bigskipamount]
  \Large{\textbf{''' + project + '''}}\\
  \normalsize{v''' + release + '''}
}
