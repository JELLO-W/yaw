...
YAW_PLUGIN_PATHS = "YAW_PLUGIN_PATHS"
    ...
    def encode(self, *args, **kwargs):
        ...
        :return: Tuple[bool, dict]: A tuple containing:
            ...
            - dict: A dictionary containing:
                - '__yaw__': A tuple containing the class name and encoded data string
        """
...
class PluginRegistrar(object):
    ...
    def scan():
        """
        Scan the plugins directory (YAW builtin and external) for plugins to register.
        ...
        modules = ["yaw.plugins." + module.replace(os.path.dirname(__file__) + "/plugins/", "") for module in modules]
        dynamic_module_import(modules, globals())

        extern_modules_paths = os.environ.get(YAW_PLUGIN_PATHS, "").split(":")
        for mod_group_idx, extern_module_path in enumerate(extern_modules_paths):
            ...
            for mod_idx, extern_module in enumerate(extern_modules):
                spec = importlib.util.spec_from_file_location(f"yaw.extern{mod_group_idx}.plugins{mod_idx}", extern_module)
                module = importlib.util.module_from_spec(spec)
                sys.modules[spec.name] = module
                spec.loader.exec_module(module)
                extern_modules = [f"yaw.extern{mod_group_idx}.plugins{mod_idx}." + extern_module.replace(
