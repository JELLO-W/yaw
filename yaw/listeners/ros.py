...
from yaw.middlewares.ros import ROSMiddleware
from yaw.encoders import JsonDecodeHook


QUEUE_SIZE = int(os.environ.get("YAW_ROS_QUEUE_SIZE", 5))
...
class ROSAudioChunkListener(ROSListener):
    ...
    def establish(self):
        ...
        """
        try:
            from yaw_ros_interfaces.msg import ROSAudioMessage
        except ImportError:
            import yaw
            logging.error("[ROS] Could not import ROSAudioMessage. "
                          "Make sure the ROS messages in yaw_extensions/yaw_ros_interfaces are compiled. "
                          "Refer to the documentation for more information: \n" +
                          yaw.__url__ + "yaw_extensions/yaw_ros_interfaces/README.md")
            sys.exit(1)
        self._queue = queue.Queue(maxsize=0 if self.queue_size is None or self.queue_size <= 0 else self.queue_size)
    ...
    def _message_callback(self, data):
        ...
        :param data: yaw_ros_interfaces.msg.ROSAudioMessage: The received message
        """
        try:
